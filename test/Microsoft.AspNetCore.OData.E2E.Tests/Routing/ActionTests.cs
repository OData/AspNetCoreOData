...
namespace Microsoft.AspNetCore.OData.E2E.Tests
{
    ...
    public class ActionTests : WebApiTestBase<ActionTests>
    {

        public ActionTests(WebApiTestFixture<ActionTests> fixture, ITestOutputHelper output)
            : base(fixture, output)
        {
    ...
    public class CustomersController : ODataController
    {
        ...
        {
            var customers = Enumerable.Range(1, 6).Select(i => new ActionTests.Customer
            {
                ID = i,
                Name = "Name " + i
            }).ToList();
        ...
        [HttpGet]
        public ActionTests.Customer Get(int key)
        {
            return new ActionTests.Customer { ID = key, Name = "Name" + key.ToString() };
        }
        ...
        public bool DoSomething(int key, ODataActionParameters parameters)
        {
            ...
            ValidateAddress(parameters["p2"] as ActionTests.Address);
            ValidateNumbers((parameters["p3"] as IEnumerable<string>).ToList());
            ValidateAddresses((parameters["p4"] as IEnumerable<ActionTests.Address>).ToList());
            Assert.Equal(ActionTests.Color.Red, parameters["color"]);

            Assert.NotNull(parameters["colors"]);
            IList<ActionTests.Color?> colors = (parameters["colors"] as IEnumerable<ActionTests.Color?>).ToList();
            Assert.NotNull(colors);

            Assert.Equal(ActionTests.Color.Red, colors[0]);
            Assert.Null(colors[1]);
            Assert.Equal(ActionTests.Color.Green, colors[2]);
        ...
        public bool MyAction(ODataActionParameters parameters)
        {
            ...
            ActionTests.Customer customer = parameters["Customer"] as ActionTests.Customer;
            Assert.NotNull(customer);
            Assert.Equal(101, customer.ID);
            Assert.Equal("Avatar", customer.Name);

            Assert.True(parameters.ContainsKey("Customers"));
            ValidateCustomers((parameters["Customers"] as IEnumerable<ActionTests.Customer>).ToList());

            return true;
        }

        private void ValidateAddress(ActionTests.Address address)
        {
        ...
        private void ValidateAddresses(IList<ActionTests.Address> addresses)
        {
        ...
        private void ValidateCustomers(IList<ActionTests.Customer> customers)
        {
            Assert.NotNull(customers);
            Assert.Equal(2, customers.Count);

            ActionTests.Customer customer = Assert.IsType<ActionTests.Customer>(customers[0]);
            Assert.NotNull(customer);
            Assert.Equal(901, customer.ID);
            Assert.Equal("John", customer.Name);

            ActionTests.SubCustomer subCustomer = Assert.IsType<ActionTests.SubCustomer>(customers[1]);
