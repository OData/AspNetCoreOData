...
namespace Microsoft.AspNetCore.OData.E2E.Tests.DollarApply
{
    public class DollarApplyTests : WebApiTestBase<DollarApplyTests>
    {
        ...
        public async Task TestGroupBySingleDynamicPropertyAsync(string queryUrl)
        {
            // Arrange & Act
            var response = await SetupAndFireRequestAsync(queryUrl);
        ...
        public async Task TestAggregateSingleDynamicPropertyAsync(string queryUrl)
        {
            ...
            var result = content.GetValue("value") as JArray;
            Assert.NotNull(result);
            var resultAt1 = Assert.Single(result);

            Assert.Equal(270, resultAt1.Value<decimal>("AverageCommission"));
        }
        ...
        public async Task TestGroupBySingleDynamicPropertyAndAggregateSingleDynamicPropertyAsync(string queryUrl)
        {
            var response = await SetupAndFireRequestAsync(queryUrl);
            ...
            var resultAt1 = Assert.IsType<JObject>(result[1]);
            Assert.Equal("Female", resultAt0.Value<string>("Gender"));
            Assert.Equal(620m, resultAt0.Value<decimal>("SumCommission"));
            Assert.Equal("Male", resultAt1.Value<string>("Gender"));
            Assert.Equal(190m, resultAt1.Value<decimal>("SumCommission"));
        }

        [Theory]
        [InlineData("default/Employees?$apply=groupby((Gender),aggregate(Salary with max as MaxSalary))")]
        [InlineData("custom/Employees?$apply=groupby((Gender),aggregate(Salary with max as MaxSalary))")]
        public async Task TestGroupBySingleDynamicPropertyAndAggregateSinglePropertyAsync(string queryUrl)
        {
            var response = await SetupAndFireRequestAsync(queryUrl);
