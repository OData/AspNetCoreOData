...
using Microsoft.AspNetCore.OData.E2E.Tests.Singleton;
...
namespace Microsoft.AspNetCore.OData.E2E.Tests
{
    ...
    public class SingletonTests : WebApiTestBase<SingletonTests>
    {

        public SingletonTests(WebApiTestFixture<SingletonTests> fixture, ITestOutputHelper output)
            : base(fixture, output)
        {
...
#region Singleton
        ...
        public async Task SingletonCRUD(string model, string singletonName)
        {
            ...
            // GET singleton
            HttpResponseMessage response = await CreateClient().GetAsync(requestUri);
            ...
            response = await CreateClient().GetAsync(requestUri + "/Revenue");
            ...
            // GET singleton
            response = await CreateClient().GetAsync(requestUri);
            result = JObject.Parse(await response.Content.ReadAsStringAsync());
        ...
        public async Task EntitySetNavigationLinkCRUD(string model, string format)
        {
            ...
            response = await CreateClient().GetAsync(requestUri + formatQuery);
            result = JObject.Parse(await response.Content.ReadAsStringAsync());
        ...
        public async Task SingletonDerivedTypeTest(string model, string singletonName, string format)
        {
            ...
            response = await CreateClient().GetAsync(requestUri + "/Location?" + formatQuery);
            ...
            response = await CreateClient().GetAsync(requestUri + "/Office?" + formatQuery);
            ...
            response = await CreateClient().GetAsync(requestUri + "?$select=Location&" + formatQuery);
            result = await response.Content.ReadAsObject<JObject>();
        ...
        public async Task SingletonQueryOptionsTest(string model, string singletonName)
        {
            ...
            var response = await CreateClient().GetAsync(requestUri + "?$select=Name");
            var result = await response.Content.ReadAsObject<JObject>();
            int i = 0;
            ...
            response = await CreateClient().GetAsync(requestUri + "?$expand=Partners($select=Name)");
            result = await response.Content.ReadAsObject<JObject>();
            ...
            response = await CreateClient().GetAsync(requestUri + "?$expand=Company($select=Name)");
            result = await response.Content.ReadAsObject<JObject>();
