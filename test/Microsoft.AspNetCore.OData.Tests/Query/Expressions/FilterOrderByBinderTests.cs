//-----------------------------------------------------------------------------
// <copyright file="FilterOrderByBinderTests.cs" company=".NET Foundation">
...
{
    public class FilterOrderByBinderTests
    {
        ...
        public void EqualityOperatorWithNull2(string productName, bool withNullPropagation, bool withoutNullPropagation)
        {
            ...
            FilterClause filter = CreateFilterNode("ProductName eq null", model, typeof(Product));
        ...
        public void BooleanOperatorNullableTypes()
        {
            ...
            (var orderByExprWithNullPropagation, var orderByExpr) = BindToExpression<Product>(orderBy,
                "",
                "");
        }
...
#region NULL  handling
        ...
        public void StringComparisons_Work(string filter, string value, bool expectedResult)
        {
            ...
            (var filterExprWithNullPropagation, var filterExpr) = BindToExpression<DataTypes>(filter);

            string a = filterExprWithNullPropagation.ToString("C#");
            string b = filterExpr.ToString("C#");

            var result = RunFilter(filterExprWithNullPropagation, new DataTypes { StringProp = value });
...
#region 'isof' in query option
        ...
        private static Expression<Func<TEntityType, bool>> Bind<TEntityType>(FilterClause filterClause, IEdmModel model, IAssemblyResolver assembliesResolver, ODataQuerySettings querySettings)
        {
            ...
            return filterBinder.BindFilter(filterClause, context) as Expression<Func<TEntityType, bool>>;
        }
        ...
        private (Expression<Func<T, bool>>, Expression<Func<T, bool>>) BindToExpression<T>(string filter, string expectedResult = null,
            string expectedResultWithNullPropagation = null, Action<ODataQuerySettings> settingsCustomizer = null) where T : class
        {
            IEdmModel model = GetModel<T>();
            FilterClause filterNode = CreateFilterNode(filter, model, typeof(T));
            IAssemblyResolver assembliesResolver = AssemblyResolverHelper.Default;
            ...
            return (filterExprWithNullPropagation, filterExpr);
        }
        ...
        private void VerifyExpression(Expression filter, string expectedExpression)
        {
            ...
            // Use `ExpressionTreeToString` extension method
            string resultExpression = filter.ToString("C#");
            Assert.True(resultExpression == expectedExpression,
                String.Format("Expected expression '{0}' but the deserializer produced '{1}'", expectedExpression, resultExpression));
        }
