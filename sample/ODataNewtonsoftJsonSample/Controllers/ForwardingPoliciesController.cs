...
namespace ODataRoutingSample.Controllers.v1
{
    ...
    public class ForwardingPoliciesController : Controller
    {
        ...
        {
            /*
BUG this request results in a 200, but the second request results in a 500
...
*/
            ...
            if (delta == null)
            {
                return StatusCode(
                    500, 
                    string.Join(Environment.NewLine, base.ModelState.Root.Errors.Select(error => $"{error.ErrorMessage}: {error.Exception.ToString()}")));
            }
