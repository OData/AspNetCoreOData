...
public class ForwardingPolicy : Policy
{
    ...
    private void ValidateForwardingPolicyRules()
    {
        ...
        foreach (PolicyRule rule in PolicyRules)
        {
            ...
            {
                throw new EntitiesMismatchException($"A {typeof(ForwardingPolicy)} contains a {typeof(PolicyRule)} that is not of type: {typeof(ForwardingRule)}");
            }
            ...
            if (TrafficForwardingType == Enums.TrafficForwardingType.Private)
            {
                ...
                {
                    throw new EntitiesMismatchException($"Rule {rule.Id} on private access forwarding policy {Id} is not a private access rule");
                }
        ...
        {
            throw new EntityValidationException("Private access forwarding policy has rules linked to more than 1 appId");
        }
    }
    #endregion
}
