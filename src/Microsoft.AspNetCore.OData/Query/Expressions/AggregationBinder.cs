...
namespace Microsoft.AspNetCore.OData.Query.Expressions
{
    ...
    public class AggregationBinder : QueryBinder, IAggregationBinder
    {
        ...
        private const string AggregationContainerProperty = "Container";
        ...
        public virtual Expression BindSelect(TransformationNode transformationNode, QueryBinderContext context)
        {
            ...
            if (groupingProperties?.Any() == true)
            {
                ...
                wrapperTypeMemberAssignments.Add(
                    ...
                    Expression.Property(Expression.Property(groupingParameter, "Key"), GroupByContainerProperty)));
            }
            ...
            if (aggregateExpressions != null)
            {
                ...
                PropertyInfo wrapperProperty = resultClrType.GetProperty(AggregationContainerProperty);
            ...
            out IDictionary<SingleValueNode, Expression> flattenedPropertiesMap)
        {
            ...
            MemberExpression containerExpression = Expression.Property(contextParameter, GroupByContainerProperty);
            ...
            for (int i = 0; i < aggregateExpressions.Count; i++)
            {
                ...
                UnaryExpression flattenedAccessExpression = Expression.Convert(
                    Expression.Property(containerExpression, "Value"),
                    type);
                containerExpression = Expression.Property(containerExpression, "Next");
            ...
            QueryBinderContext context)
        {
            ...
            PropertyInfo wrapperProperty = nestedResultType.GetProperty(AggregationContainerProperty);
            wrapperTypeMemberAssignments.Add(Expression.Bind(wrapperProperty, AggregationPropertyContainer.CreateNextNamedPropertyContainer(properties)));
                ...
                case AggregationMethod.Average:
                    {
                        ...
                        // For dynamic properties, cast back to object
                        // TODO: Test whether propertyLambda.Type can be object at this point
                        if (propertyLambda.Type == typeof(object))
                        {
                            propertyAggregateExpression = Expression.Convert(propertyAggregateExpression, typeof(object));
                        }
            ...
            switch (transformationNode.Kind)
            {
                ...
                case TransformationNodeKind.GroupBy:
                    ...
                    if (groupByClause.ChildTransformations != null)
                    {
                        ...
                        {
                            // TODO: Throw an descriptive exception - NotSupported[Child|Nested]TransformationKind?
